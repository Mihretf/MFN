...
export const GlowCard = ({ children, className = '', glowColor = 'blue', size = 'md', width, height, customSize = false }) => {
  ...
  const getStyles = () => {
    const s = {
      '--base': base, '--spread': spread, '--radius': '14', '--border': '3',
      '--backdrop': 'hsl(0 0% 60% / 0.12)', '--backup-border': 'var(--backdrop)',
      '--size': '200', '--outer': '1',
  ...
  return (
    <div ref={cardRef} data-glow style={getStyles()} className={(customSize ? '' : sizeMap[size]) + (!customSize ? ' aspect-[3/4]' : '') + ' rounded-2xl relative grid grid-rows-[1fr_auto] shadow-[0_1rem_2rem_-1rem_black] p-4 gap-4 backdrop-blur-[5px] ' + className}>
      <div data-glow></div>
      {children}
    </div>
  );
